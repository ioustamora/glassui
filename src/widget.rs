...
impl Widget for ScrollArea {
    ...
    fn render(&self, renderer: &mut GlassRenderer) {
        ...
        if w > 0 && h > 0 {
            ...
            // Draw border AFTER scissor cleared (visible on top)
            renderer.draw_rect(
                self.position - Vec2::splat(1.0),
                self.size + Vec2::splat(2.0),
                Vec4::new(0.4, 0.4, 0.45, 0.3)
            );
...
// =============================================================================
// PREMIUM WIDGETS - macOS/iOS quality with enhanced glassmorphism
// =============================================================================
...
pub struct ProgressBar {
    ...
    pub size: Vec2,
    pub value: f32,           // 0.0 to 1.0
    pub target_value: f32,    // For smooth animation
    pub animated_value: f32,  // Current animated position
    pub indeterminate: bool,
    pub indeterminate_phase: f32,
    pub show_percentage: bool,
    pub color: Option<Vec4>,  // Custom color override
...
impl Widget for ProgressBar {
    ...
    fn render(&self, renderer: &mut GlassRenderer) {
        ...
        // Track background with subtle glass effect
        renderer.draw_rect(
            self.position,
            self.size,
        ...
        if self.indeterminate {
            ...
            let ease_t = easing::ease_in_out_quart(
                (self.indeterminate_phase * 2.0 - 1.0).abs()
            );
            let x_offset = ease_t * travel;
            
            // Glow
            renderer.draw_rect(
                self.position + Vec2::new(2.0 + x_offset - 4.0, 0.0),
                Vec2::new(bar_width + 8.0, self.size.y),
                Vec4::new(bar_color.x, bar_color.y, bar_color.z, 0.3)
            );
            ...
            if fill_width > 0.0 {
                ...
                renderer.draw_rect(
                    self.position + Vec2::new(2.0, 2.0),
                    Vec2::new(fill_width, (self.size.y - 4.0) * 0.4),
            ...
            if self.show_percentage {
                ...
                let text_x = self.position.x + self.size.x + 10.0;
                renderer.draw_text(&pct, Vec2::new(text_x, self.position.y + 2.0), 16.0, theme.text);
            }
        }
    }
}
...
/// Premium iOS-style toggle switch with spring animation
pub struct Toggle {
    pub position: Vec2,
    pub size: Vec2,
    pub checked: bool,
    pub label: String,
    pub animated_t: f32,    // 0.0 = off, 1.0 = on
...
impl Widget for Toggle {
    ...
    fn handle_event(&mut self, event: &winit::event::Event<()>, mouse_pos: Vec2) -> bool {
        ...
        let track_rect = (self.position, Vec2::new(track_width, self.size.y));
        let inside = mouse_pos.x >= track_rect.0.x && mouse_pos.x <= track_rect.0.x + track_rect.1.x &&
                     mouse_pos.y >= track_rect.0.y && mouse_pos.y <= track_rect.0.y + track_rect.1.y;
    ...
    fn update(&mut self, dt: f32) {
        // Spring physics for smooth toggle animation
        let target = if self.checked { 1.0 } else { 0.0 };
        let spring_k = 180.0;   // Spring stiffness
        let damping = 12.0;     // Damping factor
    ...
    fn render(&self, renderer: &mut GlassRenderer) {
        ...
        // Track background - interpolate between off and on colors
        ...
        // Knob position with spring animation
        ...
        // Knob
        let knob_color = if self.pressed {
        ...
        renderer.draw_rect(
            Vec2::new(knob_x, knob_y),
            Vec2::new(knob_size, knob_size * 0.4),
            Vec4::new(1.0, 1.0, 1.0, 0.3)
        );
        ...
        renderer.draw_text(
            &self.label,
            self.position + Vec2::new(track_width + 12.0, 4.0),
            16.0,
            theme.text
        );
    }
...
impl RadioGroup {
    ...
    pub fn with_selected(mut self, index: usize) -> Self {
        if index < self.options.len() {
            self.selected_index = index;
            self.animated_selection = index as f32;
        }
        self
    }
...
impl Widget for RadioGroup {
    ...
    fn update(&mut self, dt: f32) {
        ...
        self.animated_selection += (target - self.animated_selection) * 12.0 * dt;
    }
    ...
    fn render(&self, renderer: &mut GlassRenderer) {
        ...
        for (i, option) in self.options.iter().enumerate() {
            ...
            // Radio outer circle (using rect as placeholder)
            ...
            renderer.draw_rect(
                Vec2::new(radio_x, radio_y),
                Vec2::new(radio_size, radio_size),
                outer_color
            );
            
            // Radio inner (background)
            renderer.draw_rect(
                Vec2::new(radio_x + 2.0, radio_y + 2.0),
                Vec2::new(radio_size - 4.0, radio_size - 4.0),
                theme.background
            );
            ...
            if is_selected {
                ...
                // Glow
                renderer.draw_rect(
                    Vec2::new(radio_x + center_offset - 2.0, radio_y + center_offset - 2.0),
                    Vec2::new(inner_size + 4.0, inner_size + 4.0),
                    Vec4::new(theme.primary.x, theme.primary.y, theme.primary.z, 0.3)
                );
                
                // Dot
                renderer.draw_rect(
                    Vec2::new(radio_x + center_offset, radio_y + center_offset),
            ...
            renderer.draw_text(
                option,
                Vec2::new(radio_x + radio_size + 12.0, item_y + (self.item_height - 16.0) / 2.0),
                16.0,
...
pub struct NumberInput {
    ...
    pub hovered_btn: Option<bool>, // true = increment, false = decrement
...
impl NumberInput {
    ...
    pub fn with_precision(mut self, precision: usize) -> Self {
        ...
        self.text_buffer = format!("{:.prec$}", self.value, prec = precision);
        self
    }
...
impl Widget for NumberInput {
    ...
    fn handle_event(&mut self, event: &winit::event::Event<()>, mouse_pos: Vec2) -> bool {
        let btn_width = 28.0;
        let dec_rect = (self.position, Vec2::new(btn_width, self.size.y));
        let inc_rect = (Vec2::new(self.position.x + self.size.x - btn_width, self.position.y), Vec2::new(btn_width, self.size.y));
        let input_rect = (Vec2::new(self.position.x + btn_width, self.position.y), Vec2::new(self.size.x - btn_width * 2.0, self.size.y));
        
        let in_dec = mouse_pos.x >= dec_rect.0.x && mouse_pos.x <= dec_rect.0.x + dec_rect.1.x &&
                     mouse_pos.y >= dec_rect.0.y && mouse_pos.y <= dec_rect.0.y + dec_rect.1.y;
        let in_inc = mouse_pos.x >= inc_rect.0.x && mouse_pos.x <= inc_rect.0.x + inc_rect.1.x &&
                     mouse_pos.y >= inc_rect.0.y && mouse_pos.y <= inc_rect.0.y + inc_rect.1.y;
        let in_input = mouse_pos.x >= input_rect.0.x && mouse_pos.x <= input_rect.0.x + input_rect.1.x &&
                       mouse_pos.y >= input_rect.0.y && mouse_pos.y <= input_rect.0.y + input_rect.1.y;
    ...
    fn update(&mut self, dt: f32) {
        ...
        if let Some(is_inc) = self.pressed_btn {
            ...
            if self.repeat_timer > 0.4 {
                ...
                self.repeat_timer -= 0.08; // Repeat rate
            }
        }
    }
    ...
    fn render(&self, renderer: &mut GlassRenderer) {
        ...
        renderer.draw_rect(self.position, Vec2::new(btn_width, self.size.y), dec_color);
        ...
        // Value text
        let text_x = self.position.x + btn_width + 8.0;
        renderer.draw_text(&self.text_buffer, Vec2::new(text_x, self.position.y + 7.0), 16.0, theme.text);
    }
}
